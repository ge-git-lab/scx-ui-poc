...
import AddDataFormBankAddition from '../component/AddDataFormBankAddition';
...
const CopySubsBankAddition = () => {
  const [CopySubsContactAdditionData, setCopySubsContactAdditionData] = useState([]);
  ...
  const fetchDataAndSetState = async () => {
      try {
        const result = await fetchData('https://t2635htwi8.execute-api.us-east-1.amazonaws.com/scx-dq-rem/dsp-copysubsbankaddition');
        setCopySubsContactAdditionData(result);
      } catch (error) {
  ...
  const handleSaveData = async (data) => {
    try {
      await saveData('https://t2635htwi8.execute-api.us-east-1.amazonaws.com/scx-dq-rem/dsp-copysubsbankaddition', data);
      fetchDataAndSetState();
    } catch (error) {
  ...
  const handleDeleteData = async (idToDelete) => {
    try {
      const result = await deleteData('https://t2635htwi8.execute-api.us-east-1.amazonaws.com/scx-dq-rem/dsp-copysubsbankaddition', idToDelete)
      console.log(result)
  ...
  const importDataInCSV = async () => {
    ...
    csvLink.download = 'copy_subs_bank_addition.csv';
    csvLink.click();
    fetchDataAndSetState();
  ...
  const handleImport = async () => {
    try {
      const response = await exportData('https://t2635htwi8.execute-api.us-east-1.amazonaws.com/scx-dq-rem/dsp-copysubsbankaddition/importexport');
    ...
    try {
      ...
      if (fileInput && fileInput.files.length > 0) {
        ...
        fileReader.onload = () => {
          ...
          console.log(csvData)
          importData('https://t2635htwi8.execute-api.us-east-1.amazonaws.com/scx-dq-rem/dsp-copysubsbankaddition/importexport', csvData);
        }
  ...
  const handleDeleteAll = async () => {
    try {
      const response = await deleteAllData('https://t2635htwi8.execute-api.us-east-1.amazonaws.com/scx-dq-rem/dsp-copysubsbankaddition/importexport', {});
  ...
  const handleUpdateProcedure = async () => {
    try {
      const result = await callProcedure('https://t2635htwi8.execute-api.us-east-1.amazonaws.com/scx-dq-rem/dsp-copysubsbankaddition/updateprocedure');
      fetchDataAndSetState();
      console.log(result)
  ...
  const handleStageProcedure = async () => {
    try {
      const result = await callProcedure('https://t2635htwi8.execute-api.us-east-1.amazonaws.com/scx-dq-rem/dsp-copysubsbankaddition/stageprocedure');
      fetchDataAndSetState();
      console.log(result)
  ...
  return (
    <div className="col-12 d-flex flex-column" style={{ minHeight: '100vh' }}>
      <Header title="Copy Subs Bank Addition" />
      <div className='button-section  mt-5 d-flex justify-content-end'>
        ...
        <button type='button' title='Update Data' className='btn-custom btn btn-custom-upload me-1' onClick={handleUpdateProcedure}>
          <FontAwesomeIcon icon={faSync} className='icon-custom edit-icon' />
        </button>
        <button type='button' title='Stage Data' className='btn-custom btn btn-custom-upload me-1' onClick={handleStageProcedure}>
      ...
      <div className='scrollable-content flex-grow-1 overflow-auto'>
        <div className='table-content mt-3'>
          <table className="table table-striped dsp-custom-table">
            ...
            <tbody>
              {CopySubsContactAdditionData.map((item) => (
                <tr className='dsp-ellipsis' key={item.subscription_id}>
                  ...
                  <td title={item.bank_account_num}>{item.bank_account_num}</td>
                  <td title={item.currency_code}>{item.currency_code}</td>
                  <td title={item.routing_num}>{item.routing_num}</td>
                  <td title={item.swift_code}>{item.swift_code}</td>
...
export default CopySubsBankAddition;
