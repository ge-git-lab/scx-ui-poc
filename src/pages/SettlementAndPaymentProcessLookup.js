...
import AddDataFormSettlementAndPaymentProcessLookup from '../component/AddDataFormSettlementAndPaymentProcessLookup';

const SettlementAndPaymentProcessLookup = () => {
  const [SettlementAndPaymentProcessLookupData, setSettlementAndPaymentProcessLookupData] = useState([]);
  ...
  const fetchDataAndSetState = async () => {
    try {
      const result = await fetchData('https://uxugef40x8.execute-api.us-east-1.amazonaws.com/dsp-scx-ref/dsp-settlement-payprocess-lookup');
      setSettlementAndPaymentProcessLookupData(result);
    } catch (error) {
  ...
  const handleSaveData = async (data) => {
    try {
      await saveData('https://uxugef40x8.execute-api.us-east-1.amazonaws.com/dsp-scx-ref/dsp-settlement-payprocess-lookup', data);
      fetchDataAndSetState();
    } catch (error) {
  ...
  const handleDeleteData = async (idToDelete) => {
    try {
      const result = await deleteData('https://uxugef40x8.execute-api.us-east-1.amazonaws.com/dsp-scx-ref/dsp-settlement-payprocess-lookup', idToDelete)
      console.log(result)
  ...
  const importDataInCSV = async () => {
    ...
    csvLink.download = 'settlement_payment_process_lookup.csv';
    csvLink.click();
    fetchDataAndSetState();
  ...
  const handleImport = async () => {
    try {
      const response = await exportData('https://uxugef40x8.execute-api.us-east-1.amazonaws.com/dsp-scx-ref/dsp-settlement-payprocess-lookup/importexport');
    ...
    try {
      ...
      if (fileInput && fileInput.files.length > 0) {
        ...
        fileReader.onload = () => {
          ...
          importData('https://uxugef40x8.execute-api.us-east-1.amazonaws.com/dsp-scx-ref/dsp-settlement-payprocess-lookup/importexport', csvData);
        }
  ...
  const handleDeleteAll = async () => {
    try {
      const response = await deleteAllData('https://uxugef40x8.execute-api.us-east-1.amazonaws.com/dsp-scx-ref/dsp-settlement-payprocess-lookup/importexport', {
        method: 'DELETE',
  ...
  return (
    <div className="col-12 d-flex flex-column" style={{ minHeight: '100vh' }}>
      <Header title="Settlement and Payment Process Look up" />
      ...
      <div className='scrollable-content flex-grow-1 overflow-auto'>
        <div className='table-content mt-3'>
          <table className="table table-striped dsp-custom-table">
            <thead>
              <tr>
                ...
                <th scope="col">PURCHASING&nbsp;SOURCE&nbsp;SYSTEM</th>
                <th scope="col">INVOICE&nbsp;SOURCE&nbsp;SYSTEM</th>
                <th scope="col">PROCESS</th>
                <th scope="col">PAY&nbsp;ON&nbsp;CODE</th>
                <th scope="col">SCX&nbsp;SETTLEMENT&nbsp;PROCESS</th>
                <th scope="col">SCX&nbsp;CUSTOM&nbsp;PAYMENT&nbsp;PROCESS</th>
                <th scope="col">ADDED&nbsp;BY</th>
                <th scope="col">ADDED&nbsp;ON</th>
                <th scope="col">CHANGED&nbsp;BY</th>
                <th scope="col">CHANGED&nbsp;ON</th>
                <th scope="col">PRIMARY&nbsp;REF&nbsp;ID</th>
                <th scope="col">ACTION</th>
              </tr>
            </thead>
            <tbody>
              {SettlementAndPaymentProcessLookupData.map((item) => (
                <tr className='dsp-ellipsis' key={item['primaryrefid']}>
                  ...
                  <td title={item.mapped}>{item.mapped}</td>
                  <td title={item.purchasing_source_system}>{item.purchasing_source_system}</td>
                  <td title={item.invoice_source_system}>{item.invoice_source_system}</td>
                  <td title={item.process}>{item.process}</td>
                  <td title={item.pay_on_code}>{item.pay_on_code}</td>
                  <td title={item.scx_settlement_process}>{item.scx_settlement_process}</td>
                  <td title={item.scx_custom_payment_process}>{item.scx_custom_payment_process}</td>
                  ...
                  <td title={item.primaryrefid}>{item.primaryrefid}</td>
                  <td>
          ...
          <AddDataFormSettlementAndPaymentProcessLookup
            show={showAddForm}
            editData={editData}
...
export default SettlementAndPaymentProcessLookup;